...
Copied from UDA: https://github.com/google-research/uda
...
from autoaugment.utils_pedcc import *
import numpy as np
...
def L2_norm(input):
    ...
    buffer = np.power(input, 2)

    # normp = torch.sum(buffer, 1).add_(1e-10)
    normp = np.sum(buffer, axis=1)
    normp = np.add(normp,1e-10)

    # norm = torch.sqrt(normp)
    norm = np.sqrt(normp)

    # _output = torch.div(input, norm.view(-1, 1).expand_as(input)) ###norm.view(-1,1)按列排列
...
def build_wrn_model(images, num_classes, feature_dim, wrn_size, update_bn=True, ):
